...
def post_hash
  {"title" => "Fake", "content" => "Fake", "person_id" => "1", "link" => "Fake", "likes" => "1", "thumb" => "Fake"}
end
...
def post_params
  ...
  params << "person_id=#{post_hash["person_id"]}"
  params << "link=#{post_hash["link"]}"
  params << "likes=#{post_hash["likes"]}"
  params << "thumb=#{post_hash["thumb"]}"
  params.join("&")
end
